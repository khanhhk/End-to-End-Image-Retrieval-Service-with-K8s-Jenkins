...
async def embed_image_with_trace(file: UploadFile = File(...)):
    with tracer.start_as_current_span("embedding-request") as request_span:
        ...
        with tracer.start_as_current_span(
            "read-image", links=[Link(request_span.get_span_context())]
        ):
            try:
        ...
        with tracer.start_as_current_span(
            "preprocess", links=[Link(request_span.get_span_context())]
        ):
            inputs = extractor(images=image, return_tensors="pt").to(DEVICE)

        with tracer.start_as_current_span(
            "model-inference", links=[Link(request_span.get_span_context())]
        ):
            with torch.no_grad():
